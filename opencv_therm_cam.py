...
def temps_to_rescaled_uints(f,Tmin,Tmax,Tpeak):
...
def camera_read(use_f:bool = True, filter_image:bool = False):
    ...
    try:
        while True:
            ...
            temp_min = 15
            temp_max = 45
            temp_peak = np.max(image)
            img=temps_to_rescaled_uints(image,temp_min,temp_max, temp_peak)
            ...
            if use_f:
                ...
                temp_peak=c_to_f(temp_peak)
                text = f'Tmin={temp_min:+.1f}F - Tmax={temp_max:+.1f}F - Tpeak={temp_peak:+.1f}F - FPS={1/(time.time() - t0):.1f} - Interpolation: {interpolation_list_name[interpolation_index]} - Colormap: {colormap_list[colormap_index]} - Filtered: {filter_image}'
            else:
                text = f'Tmin={temp_min:+.1f}C - Tmax={temp_max:+.1f}C - Tpeak={temp_peak:+.1f}F - FPS={1/(time.time() - t0):.1f} - Interpolation: {interpolation_list_name[interpolation_index]} - Colormap: {colormap_list[colormap_index]} - Filtered: {filter_image}'
            ...
            if key == ord("s"):  # If s is chosen, save an image to filec
                ...
                if (counter == 0):
                    ...
                    photo = ("img_" + str(cur_date) + "_" + str(cur_time) + ".jpg")
                    camera.capture(photo)
                    initial = os.path.abspath(photo)
                    new = '/home/pi/Final_project/Images/' + photo
                    copy = shutil.copyfile(initial,new)
                    camera.close()
                    outputs.email(photo)
                else:
                    time.sleep(.5)
                    return round(temp_peak,1)
